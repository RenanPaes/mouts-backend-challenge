...
public class UpdateSaleHandler : IRequestHandler<UpdateSaleCommand, SaleResult>
{
    ...
    public async Task<SaleResult> Handle(UpdateSaleCommand command, CancellationToken cancellationToken)
    {
        ...
        // Replace the items collection through the aggregate so discount rules and totals are enforced.
        sale.Items.Clear();
        ...
        await _mediator.Publish(new DomainEventNotification<SaleModifiedEvent>(new SaleModifiedEvent(updatedSale)), cancellationToken);
